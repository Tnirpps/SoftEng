...
class FilesListHandler final
    : public Handlers::TypedHandler<
          Gen::openapi::FileListResponse,
          userver::server::http::HttpStatus::kOk> {
  public:
    static constexpr std::string_view kName = "handler-directory-files-list";

    using Response = TypedHandler::Response;

    FilesListHandler(const userver::components::ComponentConfig &config,
  ...
  private:
    const Cache::DirectoryFilesCache& files_cache_;
